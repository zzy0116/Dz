			...
			<div class="fr">
				...
				<table cellpadding="0" cellspacing="0">
					<tr>
						...
						<td class="lpadding rpadding" name="userName"><input type="text" /></td>
...
<div class="section clearfix" >
	...
	<%for(int i=0;i<bigsections.size();i++){ %>
		...
		<%} %>
		<% count=0;m=0;%>
		<% for(int k=0;k<tiezis.size();k++){ 
     			if(bigsections.get(i).getBigId()==tiezis.get(k).getBigId()){
     				m++;
     			}	
     		} 
		if(m%2==0){
		...
		<div class="block7head clearfix"  style="margin-top:0px";>
	 	...
	 	<%for(int j=0;j<tiezis.size();j++){ %>
			...
			<%}%>
			<%count++;%>
			<%if(count>m){ %><div class="smallblocktwo fl clearfix "><%} %><% else{%>
			...
			<div class="fl" style="margin: 10px 0 10px 15px;">
				<ul>
					<li><a href="TieServlet?smallId=<%=tiezis.get(j).getSmallId() %>" class="blockhead" style="color: red;"><%=tiezis.get(j).getSmallsection() %></a> </li>
					<li>	
						主题：<%=tiezis.get(j).getSumpost() %>,贴数：<%=tiezis.get(j).getSspost() %>
					</li>
					<li>
						<%if(tiezis.get(j).getTitle()==null){tiezis.get(j).setTitle("null");}%> <%=tiezis.get(j).getTitle() %><%if(tiezis.get(j).getName()==null){tiezis.get(j).setName("null");}%><%=tiezis.get(j).getName() %><%if(tiezis.get(j).getTime()==null){tiezis.get(j).setTime(new Date());}%><%=tiezis.get(j).getTime() %>
					</li>
				</ul>
			</div>
			</div>
